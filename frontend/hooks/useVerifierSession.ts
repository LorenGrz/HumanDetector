...
import type { ServerMessage, VerifierState } from "@/types/protocol";
...
function applyMessage(state: VerifierState, message: ServerMessage): VerifierState {
  switch (message.kind) {
    ...
    case "reveal":
      return { ...state, phase: "reveal", revealText: message.text };
  }
}
...
export function useVerifierSession() {
  ...
  useEffect(() => {
    const socket = new VerifierSocket(WS_URL, {
      onMessage: (message) => setState((prev) => applyMessage(prev, message)),
      onClose: () =>
  ...
  return { state, sendFrame, restart };
}
